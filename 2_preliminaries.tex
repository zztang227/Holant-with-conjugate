...
Throughout the paper, signatures are Boolean and complex-valued unless stated
otherwise.  We write
\[
\mathbb N=\{0,1,2,\ldots\},\qquad [n]=\{1,\ldots,n\},\qquad
\mathbb C^\times=\mathbb C\setminus\{0\},
\]
and let $\mathfrak i^2=-1$.  We fix
\[
\alpha=\frac{1+\mathfrak i}{\sqrt 2}=e^{\pi\mathfrak i/4},
\]
so that $\alpha^2=\mathfrak i$.

A \emph{signature} of arity $n$ is a function
$f:\{0,1\}^n\to\mathbb C$ on an ordered list of variables
$x_1,\ldots,x_n$.  We also write $\operatorname{arity}(f)=n$ and
$f^\sigma=f(\sigma)$.  Unless a row orientation is explicitly indicated,
the truth table of $f$ is the column vector in lexicographic order.  For a
bit string $\sigma=\sigma_1\cdots\sigma_n$, let $|\sigma|=n$,
\[
\operatorname{wt}(\sigma)=\sum_{j=1}^n\sigma_j,
\]
and let $\overline{\sigma}$ be its bitwise complement.  The symbols
...
\[
\mathcal S(f)=\{\sigma\in\{0,1\}^n:f(\sigma)\ne0\}.
\]
We use $\mathscr S(f)$ and $\operatorname{supp}(f)$ synonymously.  The
signature is zero, written $f\equiv0$, if $\mathcal S(f)=\varnothing$.
For $\mathcal T\subseteq\{0,1\}^n$, the restriction $f|_{\mathcal T}$ is
the signature that agrees with $f$ on $\mathcal T$ and is zero off
$\mathcal T$.
...
\[
f=[f_0,f_1,\ldots,f_n]_n,
\]
where $f_j$ is the value on every input of weight $j$; the arity subscript
is omitted when clear.  Let
\[
\Delta_0=[1,0],\qquad \Delta_1=[0,1].
\]
For $n\ge1$, the equality signature $=_n$ is one on $0^n$ and $1^n$ and
zero elsewhere.  The binary disequality signature is
$\neq_2=[0,1,0]$.  More generally, $\neq_{2n}$ denotes the indicator of
\[
(x_1\ne x_2)\wedge(x_3\ne x_4)\wedge\cdots\wedge
(x_{2n-1}\ne x_{2n}),
\]
including any signature obtained by permuting the paired variables.  We
write $\mathcal{EQ}=\{=_n:n\ge1\}$ and
$\mathcal{DEQ}=\{\neq_{2n}:n\ge1\}$.

Let
\[
\mathscr E_n=\{\sigma\in\{0,1\}^n:\operatorname{wt}(\sigma)
\text{ is even}\},\qquad
\mathscr O_n=\{\sigma\in\{0,1\}^n:\operatorname{wt}(\sigma)
\text{ is odd}\}.
\]
...
\emph{parity} in either case.  Following the notation of~\cite{GSS26},
...
$\mathcal S(f)\subseteq\mathrm{HW}^{\bowtie}$.  Thus the macros
$\eoe,\eog,\eol,\eosg,\eosl$ stand for
$\mathrm{HW}^{=},\mathrm{HW}^{\ge},\mathrm{HW}^{\le},
\mathrm{HW}^{>},\mathrm{HW}^{<}$, respectively.  An arity-$2d$ signature
$f$ is an \emph{Eulerian-orientation signature}, or an \emph{$\eo$
...
\[
\operatorname{vec}(b)
=(b_{00},b_{01},b_{10},b_{11})^{\mathsf T}.
\]

For a vector or matrix $A$, write $A^{\mathsf T}$ for transpose,
$\overline A$ for entrywise complex conjugation, and
$A^\dagger=\overline A^{\mathsf T}$ for conjugate transpose.  For row
...
\]
For $r\ge1$, set $I_{2^r}=I_2^{\otimes r}$ and
$N_{2^r}=N_2^{\otimes r}$; in particular, $N_4=N_2^{\otimes2}$.
The groups $\mathrm{GL}_2(\mathbb C)$, $\mathbf U_2(\mathbb C)$, and
$\mathbf O_2(\mathbb R)$ denote the invertible complex, unitary, and real
orthogonal $2\times2$ matrices, respectively.  We also use
\[
\mathbf{PU}(2)=\mathbf U_2(\mathbb C)/
 \{\lambda I_2:|\lambda|=1\}
\cong\mathbf{SU}(2)/\{\pm I_2\}.
\]

A \emph{signature grid} over $\mathcal F$ is a pair
$\Omega=(G,\pi)$, where $G=(V,E)$ is a graph and $\pi$ assigns to every
$v\in V$ a signature $f_v\in\mathcal F$ of arity $\deg(v)$ together with
an ordering of its incident edges.  Its partition function is
\[
\operatorname{Holant}_{\Omega}
=\sum_{\sigma:E\to\{0,1\}}
 \prod_{v\in V}f_v(\sigma|_{E(v)}).
\]
The problem $\Holant(\mathcal F)$ asks for this value.  For a bipartite
grid with signature sets $\mathcal G$ and $\mathcal F$ on the two sides,
we write $\holant{\mathcal G}{\mathcal F}$.  Singleton braces and unions
inside a Holant expression are suppressed.  The symbols $\le_T$ and
$\equiv_T$ denote polynomial-time Turing reducibility and equivalence.

The standard counting-CSP notation is
\[
\#\operatorname{CSP}_d(\mathcal F)
:=\holant{\{=_{kd}:k\ge1\}}{\mathcal F};
\]
we omit the subscript when $d=1$.  For a set $\mathcal F$ of $\eo$
signatures, we use
\[
\ceo(\mathcal F):=\holant{\neq_2}{\mathcal F}.
\]
The notation $\holodd$ refers to Holant problems whose signature set
contains a nonzero signature of odd arity.

For $T\in\mathrm{GL}_2(\mathbb C)$ and an arity-$n$ signature viewed as
a column vector, define
\[
Tf:=T^{\otimes n}f,\qquad
T\mathcal F:=\{Tf:f\in\mathcal F\}.
\]
For a signature written as a row vector, $fT^{-1}$ means
$f(T^{-1})^{\otimes n}$.  These conventions give
\[
M_S(Tf)=T^{\otimes|S|}M_S(f)
       (T^{\mathsf T})^{\otimes(n-|S|)}.
\]
Valiant's Holant theorem states that
\[
\holant{\mathcal G}{\mathcal F}
\equiv_T
\holant{\mathcal G T^{-1}}{T\mathcal F}.
\]
We call this equivalence a \emph{holographic transformation by $T$}.
Since $(=_2)K=\neq_2$,
\[
\holant{=_2}{\mathcal F}
\equiv_T
...
\[
\widehat f:=K^{-1}f=(K^{-1})^{\otimes\operatorname{arity}(f)}f,
\qquad
\widehat{\mathcal F}:=K^{-1}\mathcal F.
\]
Thus a hat always denotes the $K^{-1}$ holographic transform; the letter
$Z$ is reserved for the Pauli matrix.  We also call
$\holant{\neq_2}{\widehat{\mathcal F}}$ the
\emph{$K$-Holant} formulation of $\Holant(\mathcal F)$.

\begin{lemma}\label{lm: conjugation and K-trans}
Let $f$ be a complex-valued signature of arity $n$.  For every
$\sigma\in\{0,1\}^n$,
\[
\widehat{\overline f}(\sigma)
=\overline{\widehat f(\overline\sigma)}.
\]
\end{lemma}
\begin{proof}
Using $K^{\mathsf T}=N_2K^{-1}$, we have
\[
\widehat{\overline f}
=(K^{-1})^{\otimes n}\overline f
=\overline{(K^{\mathsf T})^{\otimes n}f}
...
in~\cite{GSS26}.  Let
$\oplus_3(a,b,c)=a\oplus b\oplus c$, applied coordinatewise to strings.
For an $\eo$ signature $f$, and for all
$\beta,\gamma,\delta\in\mathcal S(f)$, define
\begin{align*}
f\in\POLMID
&\iff \beta\oplus\gamma\oplus\delta\in\mathcal S(f),\\
f\in\POLUP
&\iff \beta\oplus\gamma\oplus\delta
      \in\mathcal S(f)\cup\mathrm{HW}^{>},\\
f\in\POLDOWN
&\iff \beta\oplus\gamma\oplus\delta
      \in\mathcal S(f)\cup\mathrm{HW}^{<}.
\end{align*}
...
The special signature used in the arity-six argument is the affine
signature
\[
f_6(x_1,\ldots,x_6)
=\chi_{\mathscr E_6}(x)\,
...
\]
and its $K^{-1}$-transform is
\[
\widehat f_6(x_1,\ldots,x_6)
=\chi_{\mathscr E_6}(x)\,
 (-1)^{x_1x_2+x_2x_3+x_1x_3+x_1x_4+x_2x_5+x_3x_6}.
\]
Here $\chi_S$ denotes the indicator of the set $S$.

\subsection{Tractable Signatures}

We recall the standard tractable signature classes
from~\cite{realholant}.

\begin{definition}
The class $\mathscr T$ consists of all signatures that, up to a
permutation of variables, are tensor products of unary and binary
signatures.
...
$\mathbb Z_2$, and $Q\in\mathbb Z_4[x_1,\ldots,x_n]$ is multilinear of
degree at most two with even mixed coefficients.  Equivalently,
\[
Q(x_1,\ldots,x_n)
...
\]
The equation $A\mathbf x=0$ is evaluated over $\mathbb Z_2$.
The class of affine signatures is denoted by $\mathscr A$.  We say that
$f$ has \emph{affine support} if $\mathcal S(f)$ is an affine subspace of
$\mathbb Z_2^n$.
\end{definition}

For $s\in\mathbb Z$, let
\[
T_{\alpha^s}=\begin{bmatrix}1&0\\0&\alpha^s\end{bmatrix}.
\]
\begin{definition}
An arity-$n$ signature $f$ is \emph{local-affine} if, for every
$\sigma=s_1\cdots s_n\in\mathcal S(f)$,
\[
...
\begin{theorem}[Dichotomy for \#EO~\cite{GSS26}]
