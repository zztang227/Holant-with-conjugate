...
\]
For a matrix $A$, write $A^{\mathtt T}$ for transpose,
...
For a binary signature $b$, $M(b)=\left[\begin{smallmatrix}
    b(00) & b(01)\\
    b(10) & b(11)
...
$M(b)M(b)^{\mathtt T}=\lambda I_2$ for some $\lambda>0$.  We set
$\widehat{\mathcal O}=K^{-1}\mathcal O$ and
$\widehat{\mathcal U}=K^{-1}\mathcal U$. 
...
\begin{proof}
    ...
    \end{aligned}
    \]
    since $q^{2r}=b/a$. Thus $\holant{\neq_2}{\neq_4,\widehat{h},\hF}\equiv_T \holant{\neq_2}{\neq_4,=_r,\widehat{Q}\hF}$. By~\Cref{lm: disequality Holant CSP equivalence}, we have
    \[
        \#\operatorname{CSP}_r(\neq_2,\neq_4,\widehat{Q}\hF)
        \equiv_T \holant{\neq_2}{\neq_4,=_r,\widehat{Q}\hF}.
    \]
    Then by~\cite[full version, Theorem~5.3]{odd_holant_real}, $\#\operatorname{CSP}_r(\neq_2,\neq_4,\widehat{Q}\hF)$ is either polynomial-time solvable or $\#\operatorname{P}$-hard.
...
\begin{lemma}\label{lem: In conjugate closed setting disequality 4 gives hardness}
...
binary signature $b$ and distinct variables $x_i,x_j$ of $f$, their \emph{merging} or
\emph{contraction} is
\[
...
Similar to \cref{prop: invariant 1st-Orth}, we can prove:
\begin{proposition}\label{prop: invariant kth Orth}
    {\sc kth-Orth} is invariant up to conjugation and holographic transformation by $T\in \mathbf{U}_2(\mathbb{C})$.
\end{proposition}
