...
In this section, we prove that $\Holant(f_6,\mathcal{B},\mathcal{F})$ is \#P-hard when $\mathcal{F}$ doesn't satisfy condition (\ref{cond: tractable classes}).
...
\begin{lemma}\label{lm: all 12 binary available is hard}
    Suppose $\mathcal{F}$ doesn't satisfy condition (\ref{cond: tractable classes}). 
...
\begin{lemma}\label{lm: f6 is hard}
    If $\mathcal{F}$ doesn't satisfy condition (\ref{cond: tractable classes}), then $\Holant(f_6,\mathcal{F})$ is \#P-hard.
\end{lemma}
...
\begin{lemma}\label{lm: induction 2n=6}
    If $\mathcal{F}$ doesn't satisfy condition (\ref{cond: tractable classes}) and it contains a signature ${f}$ of arity $6$ such that $f\notin \mathcal{U}^\otimes$, then $\Holant(\mathcal{F})$ is \#P-hard.
\end{lemma}

\begin{proof}
    By \cref{thm:f6-equality-realization}, $\Holant(f_6,\mathcal{F})\equiv_T \Holant(\mathcal{F})$.
